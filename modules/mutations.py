...
def select_positions(n_mutations, neo_object, positions_select, select_position_params):
    '''
    Select mutable positions in each neoantigen based on a specific method.
    '''

    init_mutable_positions = list(range(len(neo_object.current_sequence)))
    if neo_object.position_mask == 'None':
        pass
    elif neo_object.position_mask == 'default':
        # mask 1st, 2nd, and C-terminus amino acids
        if 0 in init_mutable_positions:
            init_mutable_positions.remove(0)
        if 1 in init_mutable_positions:
            init_mutable_positions.remove(1)
        if (len(neo_object.current_sequence) - 1) in init_mutable_positions:
            init_mutable_positions.remove((len(neo_object.current_sequence) - 1))
    elif neo_object.position_mask == 'custom':
        if neo_object.hla in custom_mask:
            mask_splits = custom_mask[neo_object.hla].split(',')
        else:
            mask_splits = []
            mask_splits.append(1)
            mask_splits.append(2)
            mask_splits.append(len(neo_object.current_sequence))
        for mask_split in mask_splits:
            mask_index = int(mask_split) - 1
            if mask_index in init_mutable_positions:
                init_mutable_positions.remove(mask_index)
    
    if neo_object.raw_mutation_index != None:
        if neo_object.raw_mutation_index in init_mutable_positions:
            init_mutable_positions.remove(neo_object.raw_mutation_index)

    if positions_select == 'random':
        # Choose positions randomly.
        if neo_object.fix_flag == True:
            mutable_positions = np.random.choice(neo_object.ref_mutable_positions, size=n_mutations, replace=False)
        else:
            mutable_positions = np.random.choice(init_mutable_positions, size=n_mutations, replace=False)
        ...
        plddts = neo_object.current_prediction_results['plddt']

        neo_L = neo_object.lengths

        # Weights associated with each position in the neoantigen.
        # to account for termini systematically scoring worse in pLDDT.
        weights = np.array([0.25, 0.5, 0.75] + [1] * (neo_L - 6) + [0.75, 0.5, 0.25])

        # Sub-select lowest % quantile of plddt positions.
        n_potential = round(neo_L * mutate_plddt_quantile)
        ...
        if np.sum(position_weights) == np.nan or np.sum(position_weights) == 0.0 :
            ...
            if select_position_params != None :
                ...
                mutable_positions = potential_sites[ np.random.choice(range(len(potential_sites)), size=n_mutations, replace=False) ]

            else:
                position_weights = neo_object.position_weights
                # Choice of positions is biased by the user-defined position-specific weights.
                mutable_positions = np.random.choice(range(neo_object.lengths), size=n_mutations, replace=False, p=position_weights)
